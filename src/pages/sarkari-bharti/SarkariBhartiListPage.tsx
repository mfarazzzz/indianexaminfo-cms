...
export function SarkariBhartiListPage() {
  ...
  useEffect(() => { getCategories("govt-vacancy").then(setCategories).catch(() => {}); }, []);

  const load = useCallback(async () => {
    setLoading(true);
    try {
      const data = await getEntranceExams({ pillar: "govt-vacancy", search: search || undefined, categoryId: categoryId || undefined });
      setItems(data);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div><h1 className="text-xl font-semibold text-slate-900">Govt Vacancy</h1><p className="text-sm text-slate-500">{items.length} recruitments</p></div>
        <button onClick={() => navigate("/govt-vacancy/new")} className="flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-medium text-white hover:bg-blue-700"><Plus size={16} /> New Recruitment</button>
      </div>
      {/* Bulk Import/Export */}
      <BulkImportExport pillar="govt-vacancy" pillarLabel="Sarkari Bharti" onImportComplete={load} />
      ...
      : <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">{items.map((item) => (
        <div key={item.id} onClick={() => navigate(`/govt-vacancy/${item.id}`)} className="bg-white rounded-lg border border-slate-200 p-4 hover:border-blue-300 hover:shadow-sm cursor-pointer group relative">
          <div className="absolute top-2 right-2 flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-all">
            <ViewOnSiteButton pillar="govt-vacancy" category={item.category} slug={item.slug} isPublished={item.isPublished} />
