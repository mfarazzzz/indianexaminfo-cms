        ...
        {activeTab === "ai" && (
          <div>
            ...
            {[
              ...
              { key: "ai_auto_summary", label: "Auto-generate Summary Box" },
              { key: "ai_auto_faq",     label: "Auto-generate FAQs" },
            ].map(({ key, label }) => (
              <Field key={key} label={label}>
                <Toggle value={get(key, true) as boolean} onChange={(v) => set(key, v)} />
              </Field>
            ))}
            <Field label="Content Language">
              <select value={(get("ai_language","en") as string)} onChange={(e) => set("ai_language", e.target.value)}
            ...
            <Field label="Writing Tone">
              <select value={(get("ai_tone","informative") as string)} onChange={(e) => set("ai_tone", e.target.value)}
                className="rounded border border-slate-200 px-3 py-2 text-sm focus:outline-none">
